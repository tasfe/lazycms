...
class Recordset{
    ...
    function button($p1=null){
        ...
        $R = '<div class="button"><button onclick="$(this.form).checkALL(\'all\');" type="button"'.$disabled.'>'.t('system::selectall').'</button><button onclick="$(this.form).checkALL();" type="button"'.$disabled.'>'.t('system::reselect').'</button>';
        $p2 = '<button onclick="$(this.form).ajaxButton(\'delete\');" type="button"'.$disabled.'>'.t('system::delete').'</button>';
        if (!empty($p1)) {
            $R1 = explode('|',$p1);
            foreach ($R1 as $v) {
                $R2 = explode(':',$v);

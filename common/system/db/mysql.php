...
class DB_Mysql extends DBQuery {
    ...
    function __construct($config) {
        if (!function_exists('mysql_query')) {
            return throw_error(sprintf(__('Your PHP installation appears to be missing the %s extension which is required by LazyCMS.'), 'MySQL'),E_LAZY_ERROR);
        }
        if (!empty($config)) {

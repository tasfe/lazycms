...
function lazy_main(){
    ...
    if (empty($model)) {
        ...
        $ds->open();
        $ds->thead = '<tr>';
        $ds->thead.= '<th>ID) '.t('onepage/name').'</th>';
        foreach ($fields as $field=>$label) {
            $ds->thead.= '<th>'.$label.'</th>';
        }
        $ds->thead.= '<th>'.t('onepage/path').'</th><th>'.t('onepage/hits').'</th><th>'.t('onepage/digg').'</th><th>'.t('onepage/date').'</th><th>'.t('system::Manage').'</th></tr>';
        while ($rs = $ds->result()) {
            $K = null;
            foreach ($fields as $field=>$label) {
                $K.= ",'".t2js(h2c($rs[$field]))."'";
            }
            $ds->tbody("E(".$rs['id'].",'".t2js($rs['title'])."','".SITE_BASE.$rs['path']."',".(is_file(LAZY_PATH.'/'.$rs['path'])?1:0).",".$rs['hits'].",".$rs['digg'].",'".date('Y-m-d H:i:s',$rs['date'])."'{$K});");
        }
        $ds->close();
        $ds->display();
    }
...
function lazy_edit(){
    ...
    echo '<p><label>'.t('onepage/name').':</label><input help="onepage/name" class="in w400" type="text" name="title" id="title" value="'.$title.'" /><span><input type="checkbox" name="autokeys" id="autokeys" value="1" checked="checked" cookie="true" /><label for="autokeys">'.t('system::autokeys').'</label></span></p>';
    ...
    echo '<p><label>'.t('onepage/keyword').':</label><input class="in w400" type="text" name="keywords" id="keywords" value="'.$keywords.'" />&nbsp;<button type="button" onclick="$(\'#keywords\').getKeywords(\'#title\')">'.t('system::get').'</button></p>';

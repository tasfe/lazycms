...
function model_field_manage(id,params) {
	...
	$.post(LazyCMS.ADMIN_ROOT + 'model.php?action=field',params,function(data){
		...
		if (r) {
			LazyCMS.dialog({
				...
				$('button[rel=save]',this).click(function(){
					var error = [], type = $('#field_t').val(), index = 0, selector = 'input[name=l],input[name=n],input[name=so],select[name=t],[name=w]',

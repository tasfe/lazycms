...
// getDown *** *** www.LazyCMS.net *** ***
function getDown($l1,$l2=array()){
    set_time_limit(0);
    if (function_exists('curl_init')) {
        return curl($l1,$l2);
    } elseif (function_exists('socket')) {
        $method = isset($l2['method']) ? $l2['method'] : null;
        $timeout= isset($l2['timeout']) ? $l2['timeout'] : null;
        unset($l2['method'],$l2['timeout']);
        $I1 = socket($l1,$l2,$method,$timeout);
        return $I1['result'];
        ...
        if (function_exists('file_get_contents')) {
            ...
            $I2 = fopen($l1,'rb');
            $I1 = fread($I2,filesize($l1));
            fclose($I2);
            return $I1;
        }
    } else {
        return L('error/getdown');
    }
}

// socket *** *** www.LazyCMS.net *** ***
function socket($url,$params=array(),$method='GET',$timeout=5){
    if (strlen($url)==0 || !function_exists('fsockopen')) { return false; }
    $errno  = 0; $errstr = "";
    //分析Url
    $aUrl = parse_url($url);
    $host   = isset($aUrl['host']) ? $aUrl['host'] : null;
    $port   = isset($aUrl['port']) ? $aUrl['port'] : 80;
    $path   = isset($aUrl['path']) ? $aUrl['path'] : null;
    $query  = isset($aUrl['query']) ? $aUrl['query'] : null;
    $method = strtoupper(empty($method) ? 'GET' : $method);
    ...
    if (!$fp) {
        ...
        $SendStr = "{$method} {$path}{$query} HTTP/1.0\r\n";
        $SendStr .="Host:{$host}:{$port}\r\n";
        $SendStr .="Accept: */*\r\n";
        $SendStr .="User-Agent: Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)\r\n";
...
// curl *** *** www.LazyCMS.net *** ***
function curl($l1,$l2=array()){
    $I2 = curl_init($l1);
    $I3 = pathinfo($l1);
    $l2['method'] = isset($l2['method']) ? $l2['method'] : 'GET';
    curl_setopt($I2,CURLOPT_TIMEOUT,isset($l2['timeout'])?$l2['timeout']:3);
    curl_setopt($I2,CURLOPT_HEADER,isset($l2['header'])?$l2['header']:false);
    curl_setopt($I2,CURLOPT_RETURNTRANSFER,true);
    curl_setopt($I2,CURLOPT_USERAGENT,isset($l2['useragent'])?$l2['useragent']:'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)');
    curl_setopt($I2,CURLOPT_REFERER,isset($l2['referer'])?$l2['referer']:$I3['dirname']);
    if ($l2['method']=='GET') {
        curl_setopt($I2,CURLOPT_HTTPGET,true);
    } elseif ($l2['method']=='POST') {
        if (is_array($l2['argv'])) {
            $l4 = http_build_query($l2['argv']);
        } else {
            $l4 = $l2['argv'];
        }
        curl_setopt($I2,CURLOPT_POST,true);
        curl_setopt($I2,CURLOPT_POSTFIELDS,$l4);
    }
    $I1 = curl_exec($I2);
    curl_close($I2);
    return $I1;

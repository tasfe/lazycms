...
<?php echo menu($module->L('models/@title').'|'.url('System','Models').';'.$module->L('models/add').'|'.url('System','ModelEdit').';'.$module->L('models/field/@title').'|'.url('System','ModelFields','modelid='.$modelid).';'.$menu);?>
<div class="content">
    <form action="<?php echo url('System','ModelFieldsEdit');?>" method="post" class="lz_form">
        ...
        <p><label><?php echo $module->L('models/field/label/type');?></label>
            <select name="inputtype" id="inputtype" onchange="$(this).selectType('#fieldtype');">
                <?php echo System::showTypes($inputtype);?>
            </select>
        </p>
